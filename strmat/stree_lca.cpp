...
LCA_STRUCT *lca_prep(SUFFIX_TREE tree)
{
    ...
    LCA_STRUCT *lca;
    if ((lca = (LCA_STRUCT *) malloc(sizeof(LCA_STRUCT))) == NULL) return NULL;
    ...
    if ((lca->I = (unsigned int *) malloc(num_nodes * sizeof(unsigned int))) == NULL) {
        lca_free(lca);
        return NULL;
    }
    memset(lca->I, 0, num_nodes * sizeof(unsigned int));

    if ((lca->A = (unsigned int *) malloc(num_nodes * sizeof(unsigned int))) == NULL) {
        lca_free(lca);
        return NULL;
    }
    memset(lca->A, 0, num_nodes * sizeof(unsigned int));

    if ((lca->L = (STREE_NODE *) malloc(num_nodes * sizeof(STREE_NODE))) == NULL) {
        lca_free(lca);
        return NULL;
    }
