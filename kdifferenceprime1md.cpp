      ...
      void processar(){
        instanciar(); //essa chamada depende diretamente da implementação do kdifferenceInexactMatch que será utilizado
         ...
         while(j < 5){
             ...
             jr.r = c->executar(m - j);
             if(jr.r > 0){ // então há uma ocorrencia de primer
                if(jr.j > 1){//então há mais ocorrências de primer para o mesmo r
                  for(int v = 0;v <= (jr.j - 2); v++, j++)
                    ocr[j] = jr.r--;
                } else {
                  ocr[j] = jr.r;
                  j++;
...
int KdifferenceInexactMatch1Modificado::executar(int m){
   this->m = m;
   this->rowPrint = m;
   int l;
 ...
 //  fstream out;
 //  out.open("matriz.csv", ios::out | ios::trunc);
 //  out<<",";
...
//	out<<endl;
   ...
   for(int i = 1; i <= m && Imax == -1; i++){
     ...
     tt = a[prime.j + i-1];
	  ...
	  D[n] = aux;
     if(passou) Imax = i; //se a linha não foi descartada ela é a primeira

  //   out<<a[prime.j + i - 1]<<",";
 //    for(int l = 0; l <= n; l++){
//			out<<D[l]<<","; //imprimi o valor da matriz
//		}
	//	out<<endl;
   }
   prime.jr.j = Jmax-1;
   prime.jr.r = Imax;
 //  out.close();
   //return Imax == -1 ? Imax : --Imax; //por conta do índice 1 da sequencia começar em zero
   return Imax;
}
